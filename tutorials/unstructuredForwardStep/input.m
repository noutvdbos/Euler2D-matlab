...
meshFolder = "./test/unstructuredForwardStep/";      %Name of the mesh file
...
reloadMesh = false;             %Determines if the mesh is reloaded or not
...
if (~isfile(meshSaveName) || reloadMesh)
    ...
    save(meshFolder+meshSaveName,"mesh")
else
    load(meshSaveName);
end
...
t = 0;                      % inital time
tend = 2.1e-2;              % end time
dt = 5e-6;                  %initial timestep
...
u = initField(mesh,1122);
v = initField(mesh,0.0);
rho = initField(mesh,1);
p = initField(mesh,100000);
